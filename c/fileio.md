...
> **注意：**在您的本地环境运行时，请确保您有可用的 **tmp** 目录，如果不存在该目录，则需要在您的计算机上先创建该目录。
>
...
```bash
gcc /share/lesson/c/fileio-open.c && ./a.out
```
...
```bash
gcc /share/lesson/c/fileio-read.c && ./a.out
```

康康

当上面的代码被编译和执行时，它会读取上一部分创建的文件
